...
// ---- public cars search/get (with category filter) ----
app.get('/api/cars', (req, res) => {
  ...
  if (category) { where.push(`c.category = @category`); params.category = String(category); }
...
app.patch('/api/agency/me/bookings/:id', requireAuth, (req, res) => {
  ...
  const ok = new Set(['pending','approved','declined']);
  if (!ok.has(status)) return res.status(400).json({ error: 'Invalid status' });
...
// delete agency account (and related)
